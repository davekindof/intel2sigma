...
- **Upstream field renames are a correctness issue, not a frequency issue**, and are the one catalog change that cannot wait for the next quarterly pass. A stale field *ordering* makes the dropdown less convenient; a stale field *name* makes the composer emit rules that silently match nothing. Check for renames whenever the corpus pin moves. The r2026-07-01 refresh recased every Okta field to CamelCase (`eventtype` → `eventType`, `displaymessage` → `displayMessage`, `actor.alternateid` → `actor.alternateId`) and replaced `target.user.display.name` with `target.displayName` outright; `okta.yml` was corrected alongside. Okta field names reach the query untouched — no pipeline maps them — so the catalog is the only place the correct spelling exists.
