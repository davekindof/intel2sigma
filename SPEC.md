...
Tier 1 admits two `DetectionItem` shapes per L2-P1 (2026-04-28): a *field-match* item (`field` set) and a *keyword-search* item (`field=""` — Sigma's bare-list idiom). Whitespace-only field names are rejected as typos; literal-empty field is a deliberate keyword marker. The `values` list accepts `str | int | bool | None`, where `None` is the Sigma YAML-null sentinel for "match when the field is null/absent."
...
See `docs/ui.md` for the full presentation specification. Single-mode composer:

- Linear five-stage flow (Stage 0 → 4) with Next/Back navigation.
- Stage breadcrumb at the top of every stage; click to jump (backward always; forward gated by ``RuleDraft.can_advance_to_stage``).
- Live YAML preview pane on the right, updates per keystroke (300ms debounce).
- Stage 0 offers IOC paste-and-classify, the observable card grid, a freeform "Don't see your observation type?" disclosure for long-tail logsources, and a "Browse SigmaHQ" tab in the load modal that searches the bundled ~3700-rule corpus.

The original SPEC envisioned a separate "Expert mode" — single-page layout, editable raw YAML, IDE-style field picker. Pruned without ever shipping the divergent UI; see decision log entry dated 2026-04-26 for the rationale. The composer is one mode now: structured input, read-only YAML.
...
- 2026-04-26: **Pruned the Guided/Expert dual-mode story.** Original wireframe had two modes — Guided (staged Next/Back flow) and Expert (single-page IDE-style with editable raw YAML). Only Guided ever shipped; Expert was a header pill that routed to the same composer panel. Prune rationale: (a) zero tester asks for an Expert mode; (b) the breadcrumb (commit ``04d2a2a``) gives free stage-jumping which covered the headline Expert-mode benefit; (c) freeform observation entry (``f9bc057``) and the SigmaHQ corpus browse (``0b40988``) cover the "I know what I'm doing, get out of my way" cases the docs envisioned for Expert; (d) the CLI (``intel2sigma convert``) is the real power-user entry point. Removed the header toggle, the ``expert_home`` route (now redirects to ``guided_home`` so old bookmarks still work), the ``mode`` template variable, and the ``.mode-toggle*`` CSS. CLAUDE.md I-4 simplified from "no Guided YAML textarea, only Expert" to "no editable YAML, period". docs/ui.md and docs/architecture.md rewritten to drop the dual-mode language. Future revival is data-cheap if testers ask, but we'd want a real use case first.
...
- 2026-04-28: **L2-P1 load-path round-trip hardening sweep.** Four-part sweep against the 3,708-rule SigmaHQ corpus, driven by the L1 audit script (`scripts/audit_corpus_loads.py`, commit `e9a040b`). Cumulative: clean rate 88.75% → 91.64%, exceptions 146 → 0, desync / silent_data_loss held at 0. Behaviour shifts the SPEC and `core/model.py` now reflect: (a) `DetectionItem.values` widened from `list[str | int | bool]` to `list[str | int | bool | None]` to round-trip Sigma's `Field: null` idiom (e.g. macOS process-creation rules excluding events with no command line); the serializer renders `None` as YAML null. (b) `DetectionItem.field` no longer has `min_length=1`; an empty `field=""` is now the keyword-search marker (`filter_keywords: [samr, lsarpc, winreg]` — Sigma's "match if any event field contains any of these strings" idiom). The `is_keyword` property and `_field_well_formed` validator together still reject whitespace-only field names as typos. (c) Tier-1's `values_set` simplified to `bool(item.values)` — empty list is the only "values missing" trigger; the composer textarea filters blank lines server-side, so any value reaching tier-1 is explicit user intent. (d) `DetectionItem` and `DetectionItemDraft` override the project-wide `str_strip_whitespace=True` to `False` so detection values containing meaningful whitespace (`CommandLine|endswith: ' '`) round-trip without mutation. (e) Filter-only rules now compose to `not (filter_a or filter_b)` instead of returning `None` and tripping `DRAFT_CONDITION_EMPTY`. (f) `_render_condition` wraps non-atomic NOT operands in parens — Sigma's NOT binds tighter than AND/OR, so `not a or b` renders as `not (a or b)` when the operand is an OR expression. Latent precedence bug fix that would have produced wrong queries for any user-composed multi-filter rule.
